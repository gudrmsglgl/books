...
- [1.3.4 동적 라이브러리의 장단점](#id-section21)<br>
- [1.3.5 재배치: 심벌의 실행 시 주소 결정하기](#id-section22)<br>
- [1.3.6 가상 메모리와 프로그램 메모리 구조](#id-section23)<br>

[Section1.4 컴퓨터 과학에서 추상화가 중요한 이유](#id-section24)<br>
- [1.4.1 프로그래밍의 추상화](#id-section24)<br>
- [1.4.2 시스템 설계와 추상화](#id-section25)<br>
...
> $ gcc -o pro main.c /path/to/libfoo.so <br>
pro와 같은 방식으로 생성된 **실행파일**은 적재되는 시점에 동적 링크가 일어난다.

**✅ 두 번째 방식:**
- 프로그램이 먼저 실행된 후, 프로그램의 실행 시간(runtime) 동안 코드가 직접 동적 링크를 실행
  - (runtime): cpu가 프로그램을 실행하기 시작한 시점부터 실행이 완료되어 프로그램이 종료된 시점까지 시간 의미
- 실행 시간 동적 링크(runtime dynamic linking)는 실행 파일이 실행될 때까지 어떤 동적 라이브러리에 <br>
  의존하는지 알 필요가 없기 때문에 좀 더 동적인 링크 방식.
- **런타임 동적 링크 > 링크 과정을 프로그램이 실행된 이후로 미룬다**
- 이 방식에는 실행 파일 내부에 동적 라이브러리 정보가 저장되지 않음.
  - 대신 프로그래머가 코드에 특정 API를 사용하여 필요할 때마다 동적 라이브러리를 직접 동적으로 적재할 수 있다.
  - 예를 들어, 리눅스에서 dlopen, dlsym, dlclose 등의 함수들로 런타임 동적 링크를 사용
...
많은 프로그램이 C 표준 라이브러리에 의존 <br>
만약 전부 정적 라이브러리 사용 => 내용이 동일한 수많은 C 표준 라이브러리 **코드 복사본 디스크에 저장됨** => 디스크 공간이 낭비

**장점 1** <br>
위 문제를 동적 라이브러리가 해결 <br>
동적 라이브러리는 의존하는 프로그램에 갯수에 상관없이 디스크에는 **✌🏻동적 라이브러리 복사본 하나만 저장✌🏻** <br>
메모리에 적재되는 동적 라이브러리의 코드 역시 ✌🏻모든 프로세스가 하나의 코드를 공유✌🏻 때문에 하나면 충분 <br>
=> **📌 이렇게 메모리 적재와 디스크 저장에 필요한 리소스를 대폭 절약 (공유 라이브러리라고 불리는 이유)**
<br><br>

**장점 2** <br>
동적 라이브러리의 코드가 수정된다고 할지라도 <br>
**해당 동적 라이브러리만 다시 컴파일**하면 되는 것
> 실행 파일에는 동적 라이브러리에 대한 필수 정보만 저장되어 있음 => 실행 파일 전체를 다시 컴파일 (x) <br>
=> 동적 라이브러리만 새 버전으로 교체 => 다음에 실행 파일이 실행될 때 새 버전의 동적 라이브러리가 사용
이러한 특징은 프로그램 업그레이드와 버그 수정을 매우 쉽게 만들어 줌.

<br><br>

**장점 3** <br>
동적 링크가 프로그램 실행 시간에 일어나는 것도 가능 => 프로그램 기능을 쉽게 확장 <br>
**이를 프로그램에서는 플러그인(plug-in)**
> 플러그인의 구현은 사전에 함수 여러 개를 미리 정의해 두고, <br>
플러그인에서는 이 함수들을 구현. <br>
마지막으로 동적 라이브러리의 방식을 따라 플러그인을 적재하면 프로그램이 사용할 수 있도록 한다. <br>

이렇게 새로운 동적 라이브러리를 제공하기만 하면 프로그램은 곧바로 새로운 기능을 가지게 됨

<br><br>

**장점 4** <br>

여러 언어를 혼합하여 개발할 때도 매우 유용. <br>
더 높은 성능이 요구되는 부분을 c/c++를 사용하여 동적 라이브러리로 생성. <br>
다른 언어에서 동적 라이브러리를 통해 c/c++ 작성된 함수를 직접 호출

<br><br>

**단점** <br>
- 프로그램이 적재되는 시간 또는 실행 시간에 링크 => 정적 링크 보다는 성능이 약간 떨어짐 <br>
- 동적 라이브러리의 코드는 특정 메모리 주소와 독립적으로 동작 => 위치 독립 코드(PIC - position-independent code) 불림
  - 동적 라이브러리 코드는 임의의 메모리 절대 주소(absolute address)로 참조할 수 없다.
  - 예시) call 0x4004d6 # foo 함수 호출
  - 0x4004d6는 변경이 불가능한 절대적인 값
  - 주소 독립적이란 => 어떤 프로세스에서 foo 함수를 호출하던지 간에 해당 함수의 정확한 실행 주소를 찾을 수 있다는 것

<hr>
> 💡 PIC에 대한 이해!

위치 독립 코드(PIC)란?
  - 위치 독립 코드는 메모리의 어떤 주소에 로드되더라도 정상적으로 실행될 수 있는 코드입니다.
  - 동적 라이브러리는 여러 프로그램에 의해 공유되기 때문에,
  - 메모리의 어떤 위치에 로드될지 미리 알 수 없습니다. 따라서 위치 독립 코드로 컴파일되어야 합니다.

왜 임의의 메모리 절대 주소를 참조할 수 없는가?
  - 동적 라이브러리는 실행 시 메모리의 어떤 위치에든 로드될 수 있습니다.
  - 예를 들어, 프로그램 A가 동적 라이브러리를 사용할 때는 메모리 주소 0x1000에 로드되고, 프로그램 B가 동적 라이브러리를 사용할 때는 메모리 주소 0x2000에 로드될 수 있습니다.
  - 만약 동적 라이브러리가 **절대 주소(absolute address)**를 사용한다면, 다음과 같은 문제가 발생합니다:
  - 프로그램 A에서는 0x1000 주소를 참조하도록 코드가 작성되었지만, 프로그램 B에서는 0x2000 주소를 참조해야 합니다.
  - 이렇게 되면 동적 라이브러리는 한 프로그램에서만 동작하고, 다른 프로그램에서는 동작하지 않습니다.

4. 그림으로 설명
절대 주소를 사용하는 경우 (문제 발생)

```
프로그램 A:
동적 라이브러리가 0x1000에 로드됨
call 0x1000  # 라이브러리 함수 호출

프로그램 B:
동적 라이브러리가 0x2000에 로드됨
call 0x1000  # 오류! 0x1000은 프로그램 B에서 유효하지 않음
```

위치 독립 코드를 사용하는 경우 (정상 동작)
```
프로그램 A:
동적 라이브러리가 0x1000에 로드됨
call [상대 주소]  # 상대 주소를 사용하여 함수 호출

프로그램 B:
동적 라이브러리가 0x2000에 로드됨
call [상대 주소]  # 같은 상대 주소를 사용하여 함수 호출
```
위치 독립 코드는 **상대 주소(relative address)**를 사용합니다. 
상대 주소는 현재 명령어의 위치를 기준으로 계산되기 때문에, 라이브러리가 메모리의 어디에 로드되더라도 정상적으로 동작합니다.
- 이에 메모리에 단 하나의 복사본만 존재하고 해당 코드는 여러 프로세스가 공유

<hr>

- 적재할 때 동적 링크를 수행하는 프로그램은 실행 파일만으로는 실행이 불가능.
- 동적 라이브러리를 제공하지 않거나 그 버전이 호환되지 않을 경우 프로그램이 실행되지 않음
...
printf() 함수는 동적링크에 의해 사용될 수 있는데
> ldd 명령어를 사용하여 실행 파일이 어떤 동적 라이브러리에 의존하는지 확인 <br>
> ```ldd helloworld``` <br>
>   linux-vdso.so.1 => (0x00007ffee3bae000) <br>
>   libc.so => /lib64/libc.so.6 (0x00007fd1562fd000) <br>
>   /lib64/ld-linux-x86-64.so.2 (0x00007fd1566cb000) <br>
...
어셈블리어로 작성된 코드 > ✌🏻변수 정보가 전혀 없는✌🏻 대신 전부 메모리 주소를 사용 <br>

```call 0x4004d6```

이 명령어는 실행을 시작하기 위해 메모리 주소 0x4004d6 으로 이동하라는 의미 <br>
0x4004d6이 foo 함수의 기계 명령어가 위치하고 있는 주소 <br>


**🧐 링커는 0x4004d6 라는 주소를 확정을 지어야 하는데 어떻게 확정 지을 수 있을까?** <br>
이 문제는 .o 파일을 생성할 때 부터 시작. <br>
컴파일러는 일단 다음과 같이 0x00 으로 지정하여 호출한다는 사실만 기록함 <br>
``` call 0x00```
위와 같이 컴파일러가 떠넘긴 부분 => **💁🏻 링커가 채워야 함** <br>

링커는 이 call 명령어를 찾아서 뒤의 0x00을 <br>
**해당 함수가 최종적으로 실행되는 시점의 메모리 주소로 변경해야 한다는 것을 어떻게 알 수 있나?** <br>
> 🥺 이를 컴파일러가 알려줌

<br>

#### 💁🏻컴파일러가 남겨주는 Message
- 컴파일러는 ✌🏻**메모리 주소를 확정지을 수 없는**✌🏻 변수를 발견 => .relo.text 에 해당 명령어 저장
- .relo.data 에는 해당 명령어와 관련된 데이터 저장

<br> 

#### 🤖 위를 바탕으로 컴파일러는 다음과 같은 메세지를 기록
> "코드 영역(code segment)의 시작 주소(start address) 기준 오프셋(offset)이 60바이트인 위치에서 foo 심벌 발견했지만, <br>
실행 시에 어떤 주소에서 실행해야 할지 알 수 없습니다.<br> 따라서 링커님이 실행 파일을 생성할 때 이 명령어를 수정해야 합니다."

아래는 심벌 해석(symbol resolution) 단계.
(링커는 심벌 해석을 완료해야만 링크 과정에서 오류가 없다고 확신)
  
![IMG_0399](https://github.com/user-attachments/assets/12cd316b-9def-4ce7-bffd-69202ef3349d)

<br>

대상 파일이 합쳐져 실행 파일이 되면 <br>
모든 기계 명령어와 전역 변수가 프로그램 실행 시간에 위치할 메모리 주소를 결정할 수 있음 <br>
이제 foo 함수의 실행 시간 메모리 주소가 0x4004d6 라는 것을 알 수 있음 <br> 

이어서 링커는 각 대상의 .relo.text 영역(segment)을 하나씩 읽어 <br>
기계 명령어를 수정해야 하는 foo라는 심벌이 있으며,<br>
이 심벌의 코드 영역 시작 주소 기준 오프셋이 60바이트라는 것을 확인 <br>

링커는 이렇게 확인한 정보를 이용하여 실행 파일에서 해당 call 명령어를 <br>
정확히 찾고, 이동할 소스 주소를 0x00 -> 0x4004d6으로 수정할 수 있음


![IMG_0398](https://github.com/user-attachments/assets/f173bc5e-1e5d-460e-bd9e-2d1d83a53e89)


### 🧐 프로그램이 실행된 후 메모리 주소는..?

위 과정을 쭉 보면 실행 파일이 만들어질 때 링커가 relocation을 통해 메모리 주소를 확정지을 수 있다. <br>
그런데 여기서 의아한 점은 프로그램이 실행되면 그 때 프로그램이 메모리에 올라가고 <br>
이는 곧 변수나 명령어의 메모리 주소는 프로그램이 실행될 때마다 변경된다는 건데 <br>
그러면 그 주소는 실제로 실행된 시점에 알 수 있는게 맞는데 어떻게 확정을 지을 수 있는것인가..? 라는 생각을 할 수 잇는데 <br>
링커가 그러면 실행 시간 메모리 주소를 미리 알 수 있다는 의미인데 이게 어떻게 가능할까? <br> 
> 바로 가상 메모리(virtual memory)가 이를 해결해준다.
...
![스크린샷, 2025-02-16 오후 2 22 02](https://github.com/user-attachments/assets/dcf70b5c-757e-4d72-8a6d-77d85d711848)

위 그림은 <br> 
프로그램이 ✌🏻실행✌🏻되면 해당 프로그램의 프로세스(process)가 **메모리에 적재** 되는데 <br>
어떤 형태로 메모리에 적재되는지 보여줌 <br>

<br>
메모리의 상위 주소(higher address)에 스택 영역이 존재. <br>
그리고 아래에 비어 있는 큰 공간이 존재. <br>
이어서 힙 영역이 존재 (malloc 함수가 바로 힙 영역에서 메모리를 할당) <br>
마지막으로 데이터 영역과 코드 영역(code segment)이 위치 -> 두 영역은 실행 파일의 내용이 메모리에 적재되는 곳. <br>

<br>

**🧐여기서 흥미로운 부분은 코드 영역의 시작점** <br>

모든 프로그램은 실행된 후 코드 영역이 예외 없이 메모리 주소 0x400000에서 시작하는데 <br>
그렇다면 두 프로그램 A, B 동시에 실행 중이라고 가정할 떄, CPU가 메모리 주소 0x400000 에서 가져오는 <br>
기계 명령어는 프로그램 A or B?  <br> <br>

정답은 <br>
> CPU가 프로그램 A를 실행할 떄 메모리 주소 0x400000 에서 가져온 명령어는 프로그램 A에 속하고 <br>
프로그램 B를 실행할 때 메모리 주소 0x400000에서 가져온 명령어는 프로그램 B에 속한다. <br>
둘다 메모리 주소 0x400000에서 가져온 것이지만 그 데이터는 서로 동일하지 않습니다.<br>
이런 일이 발생할 수 있는 것은 바로 운영 체제의 가상 메모리 기술입니다.

#### 💁🏻 가상 메모리란?
말 그대로 물리적으로 존재하지 않은 가짜 메모리. <br>
가상 메모리는 각각의 프로그램이 실행 중일 때, ✌🏻**자기 자신이 모든 메모리를 모두 독점적**✌🏻으로 사용하고 있는 것처럼 **착각**하게 만듬. <br>
예를들어) 32비트 시스템에서는 실제로 시스템에 설치된 물리적 메모리가 얼마가 되었든 자신이 2^32 바이트, <br>
즉 4GB 메모리를 독점하고 있다고 생각함 <br>
따라서 위의 그림은 실제 물리 메모리의 형상이 아닌 논리로만 존재하는 **허상**일 뿐<br>
이는 마치 파일을 연속적으로 읽고 있다고 생각하지만, 실제로는 그 데이터가 디스크 전체에 무작위로 흩어져 있는 것과 마찬가지. <br>
<br>

아까 생긴 궁금증인 링커가 어떻게 미리 알 수 있을까라는 질문에 대한 부분이 <br>
링커가 실행 파일을 생성하자마자 실행 시 심벌의 메모리 주소를 결정할 수 있는 것도 <br>
프로그램 실행 여부와 관계없이 프로세스 메모리 구조를 알고 있기 떄문. <br>

사실상 링커는 프로그램이 실행될 때 <br>
명령어나 데이터가 실제로 존재하는 <br>
물리 메모리(physical memory)의 어디에 위치하는지 전혀 신경 쓰지 않음 <br>

그럼 실행 파일을 실행하려면 물미 메모리에 적재되어야 하는데 <br>
실행 파일의 코드 영역이 물리 메모리 주소0x80ef000에 적재된다고 가정하면, 시스템에는 다음과 같은 <br>
**사상 관계(mapping)** 가 추가됩니다. <br>

실제 운영 체제의 모든 메모리 주소에 대해 이런 사상 관계를 유지하는 것은 아니며 <br>
메모리 페이지(memory page) 단위로 관리됨. <br>
이런 사상 관계를 기록한 표를 페이지 테이블(page table)이라고 함. <br>
**각각의 프로세스**에는 ✌🏻자신만의 페이지 테이블✌🏻이 있음.

![스크린샷, 2025-02-16 오후 10 02 44](https://github.com/user-attachments/assets/ff27e121-6114-4d4c-9d44-90119af5c179)


위의 프로그램이 실행 중일 때 메모리 상태 그림을 보고 알 수 있는 사실들
1. 모든 프로세스의 가상 메모리는 표준화되어 있고 크기가 동일. 프로세스마다 각 영역의 크기가 <br> 다를 수 있지만 영역이 배치되는 순서는 동일
2. 실제 물리 메모리의 크기는 가상 메모리의 크기와는 무관하며 <br>물리 메모리에는 힙 영역, 스택 영역 등 여역 구분조차 존재하지 않는다. 단 운영체제 마다 조금씩 다를 수 있다.
3. 모든 프로세스는 자신만의 페이지 테이블을 가지고 있으며, 같은 가상 메모리 주소라도 페이지 테이블을 확인하여 서로 다른 물리 메모리 주소를 획득. <br> 이런 이유로 CPU는 동일한 가상 메모리 주소에서 서로 다른 내용을 가져올 수 있다.
...
세부 사항을 모두 나열하지 않고도 추상화를 통해 의사소통이 편함<br>
<br>
**📌추상화**는 <br>
>표현력을 크게 향상시키고 의사소통의 효율을 올려 줄 뿐만 아니라 <br>
세부 사항을 노출할 필요가 없으므로 보호할 수도 있다.
...
🥲 소프트웨어는 복잡 <br>
⚡️ 추상화를 통해 복잡도를 제어 <br>
예를들어) 각 모듈을 설계할 때 추상화된 API만 잘 제공하면 세부사항은 알 필요 없음
...
- 고급언어로 프로그래밍 하는 프로그래머 -> 기계 명령어의 세부 사항에 신경 쓸 필요가 없음
- 입출력(input/output) 장치는 -> 파일(file)로 추상화되어 트랙(track)과 섹터(sector)에 세부 사항은 신경 슬 필요가 없음
- 실행 중인 프로그램 -> 프로세스로 추상화
  - (프로그램 작성 시 자신의 프로그램이 **CPU를 독점한다고 가정할** 수 있기 때문에 단일 CPU 시스템에서도 수많은 프로세스가 동시에 실행될 수 있음
- 물리 메모리 파일 -> 가상 메모리로 추상화 <br>
  - (물리 메모리 크기가 서로 다른 경우에도 자신의 프로그램이 표준적이고 동일한 크기의 메모리에 독점적으로 접근 가정
  - mmap 작동 방식 이용 -> 가상 메모리를 사용하더라도 실제 메모리를 읽고 쓰는 것처럼 쉽게 파일을 조작)
- 프로세스와 프로세스에 종속적인 실행 환경 -> 컨테이너(container)로 추상화
  - 더 이상 개발 환경과 실제 배포 환경의 차이를 걱정할 필요가 없음
- CPU, 운영 체제, 응용 프로그램(application)은 -> 가상머신으로 묶어(packaging) 추상화
  - 여러가지 하드웨어를 구입하여 직접 운영체제 설치하고 프로그램 구성하고 서버 환경을 운영할 필요 X
  - 가상 머신은 일종의 데이터로서 빠르게 복제가 가능 -> 한 손으로 수많은 서버를 운영하고 유지하고 관리 -> 클라우드 컴퓨팅

**🧐 개발자는 저수준 계층에 관심을 가질 필요가 전혀 없을까?** <br>
**⭐️ 저수준 계층에 대한 철저한 이해는 고급 프로그래머를 남드로가 구분 짓는 특징 중 하나**
