...
- [8. Describe the Fragment lifecycle](#id-section9)<br>
...
Fragment는 액티비티와 별도의 생명주기를 가지며, 화면에 추가·제거·전환될 때마다 다양한 상태를 거친다.  
각 단계에서 리소스 관리, UI 일관성, 사용자 반응성을 효과적으로 처리해야 한다.

#### 주요 단계

- **onAttach()**
  - 프래그먼트가 부모 액티비티에 연결될 때 최초로 호출
  - 액티비티의 **✌🏻컨텍스트에 접근✌🏻** 가능

- **onCreate()**
  - 프래그먼트가 생성될 때 호출
  - UI는 아직 생성되지 않았으며, 필수 컴포넌트 초기화나 저장된 상태 복원에 사용

- **onCreateView()**
  - 프래그먼트의 UI가 처음 그려질 때 호출
  - 레이아웃을 인플레이트하고(LayoutInflater), 루트 뷰를 반환

- **onViewStateRestored()**
  - 프래그먼트의 뷰 계층이 생성되고, 저장된 상태가 뷰에 복원된 후 호출

- **onViewCreated()**
  - 프래그먼트의 뷰가 생성된 직후 호출
  - UI 컴포넌트 초기화, 사용자 상호작용 로직 설정에 사용

- **onStart()**
  - 프래그먼트가 사용자에게 보이기 시작
  - 액티비티의 onStart()와 유사, <br>아직 ✌🏻포그라운드는 아님✌🏻

- **onResume()**
  - 프래그먼트가 완전히 활성화되어 포그라운드에서 사용자와 상호작용 가능

- **onPause()**
  - 프래그먼트가 포그라운드를 벗어나지만 여전히 화면에 보임
  - 포커스를 잃기 직전, 일시 중지해야 할 작업 처리

- **onStop()**
  - 프래그먼트가 더 이상 사용자에게 보이지 않음
  - 화면에서 사라질 때 불필요한 작업 중단

- **onSaveInstanceState()**
  - 프래그먼트가 소멸되기 전에 UI 관련 상태 데이터를 저장

- **onDestroyView()**
  - 프래그먼트의 뷰 계층이 제거될 때 호출
  - 뷰 관련 리소스 정리, 어댑터 해제, 참조 null 처리 등

- **onDestroy()**
  - 프래그먼트 자체가 소멸될 때 호출
  - 모든 리소스 최종 정리(아직 액티비티에는 연결됨)

- **onDetach()**
  - 프래그먼트가 부모 액티비티에서 완전히 분리될 때 호출
  - 생명주기 종료, 모든 연결 해제

--- 
#### 실전 질문 (Practical Questions)

**Q) onCreateView()와 onDestroyView()의 목적은 무엇이며, 이 메서드에서 뷰 관련 리소스를 적절히 관리하는 것이 왜 중요한가?**

- **onCreateView()** 는 프래그먼트의 UI를 처음 생성할 때 호출되며, 레이아웃을 인플레이트하고 루트 뷰를 반환한다.  
  이 단계에서 뷰 바인딩, 어댑터 연결 등 UI 초기화 작업을 수행한다.

- **onDestroyView()** 는 프래그먼트의 뷰 계층이 제거될 때 호출된다.  
  이 시점에 어댑터 해제, 뷰 바인딩 객체 null 처리 등 뷰와 관련된 리소스를 반드시 정리해야 한다.

**적절한 리소스 관리가 중요한 이유:**  
- 프래그먼트의 뷰는 여러 번 생성·제거될 수 있으므로, 뷰 관련 리소스를 onDestroyView()에서 해제하지 않으면 메모리 누수나 예기치 않은 동작이 발생할 수 있다.
- 뷰와 관련된 참조를 남겨두면, 프래그먼트가 화면에서 사라진 뒤에도 리소스가 해제되지 않아 앱 성능 저하 및 크래시 위험이 높아진다.
